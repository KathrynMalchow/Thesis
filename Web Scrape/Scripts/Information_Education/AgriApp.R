...
url = "https://play.google.com/store/apps/details?id=com.criyagen&hl=en&showAllReviews=true"
remDr$navigate(url)
...
# 1) Reviewer Name
names_AgriApp = html_obj %>% html_elements(".kx8XBd .X43Kjb") %>% html_text()

# 2) Number of Stars
stars_AgriApp = html_obj %>% html_elements(".kx8XBd .nt2C1d [role='img']")%>% 
...
AgriApp = tibble(
  ...
  stars = stars_AgriApp, 
  dates = dates_AgriApp, 
  reviews = reviews_AgriApp) 
saveRDS(AgriApp, 'Objects/Information_Education/AgriApp.RDS')
write_csv(AgriApp, 'data/Information_Education/AgriApp.csv')
