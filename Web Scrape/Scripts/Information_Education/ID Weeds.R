...
url = "https://play.google.com/store/apps/details?id=com.extension.idweeds&hl=en&utm_source=downloadatoz.com&showAllReviews=true"
remDr$navigate(url)
...
# 1) Reviewer Name
names_IDWeeds = html_obj %>% html_elements(".kx8XBd .X43Kjb") %>% html_text()

# 2) Number of Stars
stars_IDWeeds = html_obj %>% html_elements(".kx8XBd .nt2C1d [role='img']")%>% 
...
# 3) Date of Review
dates_IDWeeds = html_obj %>% html_elements(".kx8XBd .p2TkOb") %>% 
  html_text() %>% 
  # Convert to a Date
  mdy()

# 4) Full Text of the Review
reviews_IDWeeds = html_obj %>% html_elements(".UD7Dzf") %>% html_text() 
###Deal with the "Full Review" Issue where text is duplicated
reviews_IDWeedsr = if_else(
  #If the review is truncated
  str_detect(reviews_IDWeeds, '\\.\\.\\.Full Review'),
  #Grab all the Text After the string '...Full Review'
  str_sub(reviews_IDWeeds, 
          start = str_locate(reviews_IDWeeds, '\\.\\.\\.Full Review')[, 2]+1
  ),
  #Else remove the leading space from the review as is
  str_trim(reviews_IDWeeds))
...
IDWeeds = tibble(
  ...
  stars = stars_IDWeeds, 
  dates = dates_IDWeeds, 
  reviews = reviews_IDWeeds) 
saveRDS(IDWeeds, 'Objects/Information_Education/IDWeeds.RDS')
write_csv(IDWeeds, 'data/Information_Education/IDWeeds.csv')
