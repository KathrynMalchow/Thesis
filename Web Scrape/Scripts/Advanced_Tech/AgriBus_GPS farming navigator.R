...
url = "https://play.google.com/store/apps/details?id=com.agri_info_design.AgriBusNavi&showAllReviews=true"
remDr$navigate(url)
...
# 1) Reviewer Name
names_AgriBus = html_obj %>% html_elements(".kx8XBd .X43Kjb") %>% html_text()

# 2) Number of Stars
stars_AgriBus = html_obj %>% html_elements(".kx8XBd .nt2C1d [role='img']")%>% 
...
# 3) Date of Review
dates_AgriBus = html_obj %>% html_elements(".kx8XBd .p2TkOb") %>% 
  html_text() %>% 
  # Convert to a Date
  mdy()
...
reviews_AgriBus = if_else(
  ...
  str_detect(reviews_AgriBus, '\\.\\.\\.Full Review'),
  #Grab all the Text After the string '...Full Review'
  str_sub(reviews_AgriBus, 
          start = str_locate(reviews_AgriBus, '\\.\\.\\.Full Review')[, 2]+1
  ),
  #Else remove the leading space from the review as is
  str_trim(reviews_AgriBus))
...
AgriBus = tibble(
  ...
  stars = stars_AgriBus, 
  dates = dates_AgriBus, 
  reviews = reviews_AgriBus) 
saveRDS(AgriBus, 'Objects/Advanced_Tech/AgriBus.RDS')
write_csv(AgriBus, 'data/Advanced_Tech/AgriBus.csv')
