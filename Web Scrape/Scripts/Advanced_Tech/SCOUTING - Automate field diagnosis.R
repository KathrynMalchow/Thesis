...
url = "https://play.google.com/store/apps/details?id=com.bayer.cs.scouting&showAllReviews=true"
remDr$navigate(url)
...
# 1) Reviewer Name
names_SCOUTING = html_obj %>% html_elements(".kx8XBd .X43Kjb") %>% html_text()

# 2) Number of Stars
stars_SCOUTING = html_obj %>% html_elements(".kx8XBd .nt2C1d [role='img']")%>% 
...
# 3) Date of Review
dates_SCOUTING = html_obj %>% html_elements(".kx8XBd .p2TkOb") %>% 
  html_text() %>% 
  # Convert to a Date
  mdy()

# 4) Full Text of the Review
reviews_SCOUTING = html_obj %>% html_elements(".UD7Dzf") %>% html_text() 
###Deal with the "Full Review" Issue where text is duplicated
reviews_SCOUTING = if_else(
  #If the review is truncated
  str_detect(reviews_SCOUTING, '\\.\\.\\.Full Review'),
  #Grab all the Text After the string '...Full Review'
  str_sub(reviews_SCOUTING, 
          start = str_locate(reviews_SCOUTING, '\\.\\.\\.Full Review')[, 2]+1
  ),
  #Else remove the leading space from the review as is
  str_trim(reviews_SCOUTING))


#Put into table
SCOUTING = tibble(
  names = names_SCOUTING, 
  stars = stars_SCOUTING, 
  dates = dates_SCOUTING, 
  reviews = reviews_SCOUTING) 
saveRDS(SCOUTING, 'Objects/Advanced_Tech/SCOUTING.RDS')
write_csv(SCOUTING, 'data/Advanced_Tech/SCOUTING.csv')
