...
url = "https://play.google.com/store/apps/details?id=org.landpotential.lpks.landcover&gl=DE&showAllReviews=true"
remDr$navigate(url)
...
# 1) Reviewer Name
names_LandPKS = html_obj %>% html_elements(".kx8XBd .X43Kjb") %>% html_text()

# 2) Number of Stars
stars_LandPKS <- html_obj %>% html_elements(".kx8XBd .nt2C1d [role='img']")%>% 
...
# 3) Date of Review
dates_LandPKS <- html_obj %>% html_elements(".kx8XBd .p2TkOb") %>% 
  html_text() %>% 
  # Convert to a Date
  mdy()

# 4) Full Text of the Review
reviews_LandPKS <- html_obj %>% html_elements(".UD7Dzf") %>% html_text() 
###Deal with the "Full Review" Issue where text is duplicated
reviews_LandPKS <- if_else(
  #If the review is truncated
  str_detect(reviews_LandPKS, '\\.\\.\\.Full Review'),
  #Grab all the Text After the string '...Full Review'
  str_sub(reviews_LandPKS, 
          start = str_locate(reviews_LandPKS, '\\.\\.\\.Full Review')[, 2]+1
  ),
  #Else remove the leading space from the review as is
  str_trim(reviews_LandPKS))
...
LandPKS = tibble(
  ...
  stars = stars_LandPKS, 
  dates = dates_LandPKS, 
  reviews = reviews_LandPKS) 
saveRDS(LandPKS, 'Objects/Data_Collection_Management/LandPKS.RDS')
write_csv(LandPKS, 'data/Data_Collection_Management/LandPKS.csv')
